...
async function measureTTFD(menuText, viewText, page) {
  ...
  // Select the 3rd item (index 2)
  thirdOption = dropdownPanel.locator('li[role="option"]').nth(4);
  await thirdOption.click();
  ...
  const providerLink = frameLocator.getByText("ILANA W HULL", { exact: false });
  ...
  const networksLink = frameLocator.locator('tr:has-text("ILANA W HULL") a.networklink >> nth=1');
  ...
  await npiInput.fill('1508963026');
  ...
  const providerLink21 = frameLocator.getByText("JULIE WILSON CHILDERS", { exact: false });
  ...
  // Locate the Networks link for the specific provider (YONINA MAR's row)
  const networksLinktwo = frameLocator.locator('tr:has-text("JULIE WILSON CHILDERS") a.networklink >> nth=1');
  ...
  const resultTableone2 = frameLocator12.locator("table").first(); // Use iframe context
  await expect(resultTableone2).toBeVisible({ timeout: 30000 });
  
  
  // Step 3: Now look for the sorter
  const networkSorter2 = frameLocator12.locator('div.ant-table-column-sorters:has-text("Networks")');
...
test("Login and measure TTFD for multiple clients with Excel Output", async ({
  ...
  const filePath = `./ttfd_output_IBX_${formattedDate}.xlsx`;
